...
def inference(
    model, cfgs, rays_xyz: torch.tensor, z_vals: torch.tensor, rays_d=None, sun_d=None
):
    """
    Runs shadow-nerf model using a batch of input rays
...
class ShadowNeRF(torch.nn.Module):
    def __init__(
        ...
        self.number_of_outputs = (
            8  # rgb (3) + sigma (1) + sun visibility (1) + rgb from sky color (3)
        )
